...
| Legion HQ = référence de la certification (21/09/2026) | Legion HQ (legionhq2.com) est la référence retenue. `node scripts/audit-legionhq.mjs --write` (réseau, option `--cache`) télécharge sa base de cartes, l'apparie aux cartes de l'appli (`scripts/data/legionhq-aliases.json`), écrit le rapport `docs/audit/recoupement-legionhq.md` et la référence `src/data/legionhqReference.json`. Dans l'écran de certification : la **vitesse imprimée manquante est préremplie** d'après Legion HQ (à confirmer sur la carte), et les écarts (rang, figurines, PV, courage, défense, adrénalines d'attaque et de défense, dés et portées des armes, **mots-clés de la carte et de ses armes avec leurs valeurs**, table `scripts/data/legionhq-keyword-map.json`) sont signalés par une pastille, un panneau et le filtre « À confirmer ». Un écart déjà tranché sur le visuel s'ajoute à `src/data/legionhqTriage.json` pour ne plus être signalé (Legion HQ applique des errata ; le visuel imprimé fait foi). `audit-takras-stats.mjs` fait le même recoupement avec Legion Helper. | `audit-legionhq.mjs`, `audit-takras-stats.mjs`, `test-assistant-parcours.mjs` (scénario « vitesse préremplie ») | Limites : les conversions d'adrénaline portées par une arme ou une amélioration et les textes d'effet n'existent pas chez Legion HQ, ils restent contrôlés sur le visuel.
