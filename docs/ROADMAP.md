...
- [x] (22/09/2026) Améliorations hors mots-clés entièrement dans le moteur : Retranchement (dés de couvert rouges, dans la barre de couvert), Kraken (saisie des figurines vaincues → dés d'attaque améliorés dans la réserve), Capitaine Clone Rex (action gratuite offerte à l'unité, annulable), Présence Inspirante (le courage du porteur alimente le contrôle de panique), Postures Offensive/Défensive (action Viser/Esquiver = 2 pions, pion opposé non dépensable pendant l'attaque, face déterminée par la carte retournée), Programmation Prime d'IG-11 (Perforant 1 ou Suppressif selon le rang de la cible Prime), Charges à Protons/Soniques (Assaut 1 aux autres armes à distance). Cartes à jet de dés (Hunter, Fusil Amban, Tireurs embusqués, Médecin, Astromech instable, Tranquillité, Entraînement de l'Inquisitorius) : le résultat du dé se choisit et la Suppression est appliquée/retirée automatiquement, la blessure est rappelée pour la table. Reste en rappel justifié : Brouilleur Comms, Ordres Stricts, Système de Commandement, Loyauté Programmée, Wrecker, Concealments, Gantelets de Cad Bane, Programmation Nanny.
...
- [x] (22/09/2026) Refonte de la fiche de certification : chaque champ affiche « Appli » et « Legion HQ » au-dessus et le contrôle en dessous (bouton « Utiliser HQ » quand ils diffèrent, « carte fait foi » pour un écart déjà tranché) ; UNE seule liste de mots-clés par carte (armes comprises) ; sur une carte à plusieurs armes, l'appli demande sur quelle(s) arme(s) chaque mot-clé d'arme s'applique (bascule par arme, valeur par arme pour Impact X…), rattachement automatique s'il n'y a qu'une arme ; une seule validation par carte (plus de validations fragmentaires). Import Tabletop : la page d'import compte aussi les cartes sans certification complète et ouvre la certification sur « Dans mes listes ». Tests : comparaison des dés indépendante de l'ordre (échec de la CI après la certification du DH-447).
...
- [x] (26/09/2026) Passe visuelle de l’Assistant alignée sur la maquette : en-tête compact, suppression du grand bandeau d’étapes et de la table holographique, textes introductifs retirés du choix d’unité, portraits harmonisés, scanlines atténuées, état sélectionné renforcé et barre d’action fixée au bas de l’écran. Les transitions remettent désormais le défilement en haut. Le menu Assistant ne garde que Suivi de partie (en premier), Phases du round, Mode rapide et Retour ; « Tester mes listes » et « Cartes certifiées » sont déplacés sous Listes dans Gestion Armée avec ouverture directe des écrans existants. Validation : 60 scénarios Assistant, contrats et build complet.
