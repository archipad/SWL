...
assert.match(app, /function pickerGrid\(units\)\{const n=units\.length,cols=4;/, 'La grille de sélection affiche quatre cartes larges et basses par rangée (demande utilisateur du 25/09/2026, maquette)')
...
assert.match(app, /\[warning,range,fire,targeting,arsenal,distract,weapons,pool,poolNote\]\.filter\(Boolean\)\.forEach\(element=>\{anchor\.after\(element\);anchor=element\}\)/, 'Étape 1 : la portée est en haut, puis le Contrôle de Tir, puis les contrôles de ciblage, puis les armes, et les dés à lancer tout en bas, sous les armes')
...
assert.match(index, /resolver-polish\.css\?v=35/, 'La feuille d’harmonisation des écrans de résolution doit être chargée')
...
assert.ok(Number(index.match(/app\.js\?v=(\d+)/)?.[1]) >= 163, 'Le verrou de navigation de la certification exige une version JavaScript au moins égale à 163')
