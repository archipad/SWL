...
assert.equal(engine.weaponEligible('melee-2', 'melee'), true)
assert.equal(engine.weaponEligible('melee-2', 1), true)
assert.equal(engine.weaponEligible('melee-2', 2), true)
assert.equal(engine.weaponEligible('melee-2', 3), false)
...
assert.deepEqual({ ...engine.rallyState({ suppression: 3, block: 0, surge: 0, courage: 2, commanderCourage: 4 }) }, { before: 3, dice: 3, removed: 0, remaining: 3, courage: 4, suppressed: true, panicked: false })
...
assert.deepEqual({ ...engine.rallyState({ suppression: 3, block: 0, surge: 0, courage: null }) }, { before: 0, dice: 0, removed: 0, remaining: 0, courage: null, suppressed: false, panicked: false })
assert.deepEqual({ ...engine.rallyState({ suppression: 3, block: 0, surge: 0, courage: 2, suppressionImmune: true }) }, { before: 0, dice: 0, removed: 0, remaining: 0, courage: null, suppressed: false, panicked: false })
assert.deepEqual({ ...engine.rallyState({ suppression: 0, block: 4, surge: 3, courage: 2 }) }, { before: 0, dice: 0, removed: 0, remaining: 0, courage: 2, suppressed: false, panicked: false })
const overkill = engine.allocateWounds([{ id: 'unique', maxWounds: 2, wounds: 0 }], 5)
assert.deepEqual(overkill.models.map(model => [model.id, model.wounds, model.defeated]), [['unique', 2, true]])
assert.equal(overkill.remaining, 0)
assert.equal(overkill.overflow, 3)
assert.deepEqual({ ...engine.moraleState({ currentSuppression: 9, gainedSuppression: 3, courage: 1, vehicle: true }) }, { current: 0, gained: 0, total: 0, courage: null, suppressed: false, panicThreshold: null, panicRisk: false })
assert.deepEqual({ ...engine.moraleState({ currentSuppression: 4, gainedSuppression: 2, courage: null }) }, { current: 0, gained: 0, total: 0, courage: null, suppressed: false, panicThreshold: null, panicRisk: false })
assert.deepEqual({ ...engine.moraleState({ currentSuppression: 4, gainedSuppression: 2, courage: 2, suppressionImmune: true }) }, { current: 0, gained: 0, total: 0, courage: null, suppressed: false, panicThreshold: null, panicRisk: false })
