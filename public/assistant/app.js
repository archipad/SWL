...
const codexPortraits={'darth vader dark lord of the sith':'darth-vader.png','stormtroopers':'stormtroopers-banner.png','snowtroopers':'snowtroopers.webp','scout troopers':'scout-troopers-hd.webp','at st':'at-st-hd.webp','imperial officer':'imperial-officer.png','rebel troopers':'rebel-troopers-hd.webp','luke skywalker':'luke-skywalker.png','rebel commandos strike team':'rebel-commandos-hd.webp','rebel commandos':'rebel-commandos-hd.webp','at rt':'at-rt-hd.webp','rebel officer':'rebel-officer.png'};
const tilePortrait=name=>{const file=window.SWL_CODEX_PORTRAITS?.[norm(name)]||codexPortraits[norm(name)];return file?`../codex/portraits/${file}`:''};
function unitModelsTotal(entry){const base=certifiedUnitStats(entry)?.baseModels;if(!Number.isInteger(base))return 0;return base+(entry.unit.upgrades||[]).reduce((sum,card)=>sum+(Number(profileFor(card.name)?.addedModels)||0),0)}
const tileMini='<svg class="tile-mini" viewBox="0 0 12 20" aria-hidden="true"><circle cx="6" cy="3.4" r="2.6"/><path d="M2 8.6h8l-1 6H7.2L6 19.4 4.8 14.6H3z"/></svg>';
function tile(e){const state=stateFor(e),stats=certifiedUnitStats(e),morale=stats?.courage?engine.moraleState({currentSuppression:state.suppression,courage:stats.courage}):null,lost=defeated(e),models=unitModelsTotal(e),portrait=tilePortrait(e.unit.name);return `<button data-faction="${factionThemeForArmy(e.army)}" class="unit-tile ${lost?'unit-defeated':''} ${state.standby?'has-standby':''}" data-id="${e.id}"><span class="tile-head">${rankMark(e)}<b class="rank-label">${rankLabels[e.rank]}</b></span><span class="tile-visual"><img class="${portrait?'art-portrait':'art-card'}" src="${portrait||imageFor(e.unit.name)}" alt="" onerror="this.style.visibility='hidden'">${rankMark(e)}</span><span class="tile-body"><strong>${entryName(e)}</strong><span class="tile-meta">${models?`<span class="tile-minis" aria-hidden="true">${tileMini.repeat(Math.min(models,10))}</span><small class="tile-count">${models} figurine${models>1?'s':''}</small>`:''}${state.suppression||state.standby?`<span class="unit-status"><b>${stats?.courage===null?'Suppression : immunisée':`${state.suppression} suppression`}</b>${state.standby?'<b class="standby-ready">● ATTENTE DISPONIBLE</b>':''}${lost?'<b class="danger">UNITÉ VAINCUE</b>':morale?.panicRisk?'<b class="danger">Ralliement · risque de panique</b>':morale?.suppressed?'<b class="warning">Démoralisée · ralliement</b>':''}</span>`:''}<svg class="tile-chevron" viewBox="0 0 24 24" aria-hidden="true"><path d="M9 5l7 7-7 7"/></svg></span></span>${lost?'<span class="defeated-overlay" aria-hidden="true"><b>☠</b></span>':''}</button>`}
...
document.addEventListener('click',event=>{if(event.target.closest?.('#nextAttack')&&typeof stepIssue==='function'){const issue=stepIssue();if(issue){nextAttackDenied=true;nextAttackDeniedIssue=issue}else resetViewportAfterNavigation()}},true);
...
function decorateTacticalResolution(){
  ...
  center.querySelectorAll('.weapon-choice').forEach(choice=>{const key=choice.querySelector('[data-key]')?.dataset.key,input=choice.querySelector('[data-count]');if(!key||!input)return;const modelsNote=key.startsWith(norm(attacker.unit.name)+':')?squadModelsNote(attacker):'';if(modelsNote&&!choice.querySelector('.models-note'))choice.querySelector('.weapon-copy')?.insertAdjacentHTML('beforeend',modelsNote);const note=document.createElement('small');note.className='autofill-note';note.textContent=attackState.manualCounts?.[key]?'AJUSTÉ MANUELLEMENT':'PRÉREMPLI · MODIFIABLE';/* Idempotent : cette décoration peut s'exécuter deux fois sur le même écran, ce qui doublait l'étiquette (constaté le 25/09/2026). */choice.querySelectorAll('.autofill-note').forEach(old=>old.remove());choice.querySelector('.count-control')?.prepend(note)});
...
/* Les écrans de contrôle restent accessibles depuis Gestion Armée, où leurs
   raccourcis ont été déplacés. Le déclenchement passe par les boutons internes
   conservés dans le DOM afin de réutiliser exactement la logique existante. */
window.addEventListener('load',()=>{const requested=new URLSearchParams(location.search).get('open');if(requested==='readiness')$('#gameReadiness')?.click();if(requested==='certification')$('#certification')?.click()});
