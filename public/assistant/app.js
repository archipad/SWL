const $=s=>document.querySelector(s);const root=$('#app');let stage=1,attacker=null,defender=null;
...
const keywords=read('swl.keywords.v1',window.SWL_REFERENCE?.keywords||[]),tags=read('swl.card-tags.v1',window.SWL_REFERENCE?.tags||{});const norm=s=>(s||'').normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLowerCase().replace(/[^a-z0-9]+/g,' ').trim();
const displayName=name=>window.SWL_REFERENCE?.names?.[norm(name)]||name;
const specialImages={'at st':'tr-tt.jpg','at rt':'tl-tt.jpg','cm o 93 trooper':'cm-o93-trooper.jpg','mo dk power harpoon':'mo-dk-power-harpoon.jpg','rebel veterans':'rebel-veterans.jpg','rebel troopers':'rebel-troopers.jpg','t 47 airspeeder':'t-47-airspeeder.jpg'};const imageFor=name=>`../cards/${specialImages[norm(name)]||norm(name).replace(/ /g,'-')+'.jpg'}`;
...
function upgradeGallery(entry){const upgrades=entry.unit.upgrades||[];if(!upgrades.length)return `<section class="upgrades"><div class="section-heading"><span>AMÉLIORATIONS ÉQUIPÉES</span><b>0</b></div><p class="empty-upgrades">Cette unité n’a aucune amélioration dans la liste importée.</p></section>`;return `<section class="upgrades"><div class="section-heading"><span>AMÉLIORATIONS ÉQUIPÉES</span><b>${upgrades.length}</b></div><div class="upgrade-grid">${upgrades.map((up,index)=>{const defs=definitionsFor(up.name),fr=displayName(up.name);return `<article class="upgrade-card"><button class="upgrade-visual" data-card-name="${fr.replace(/"/g,'&quot;')}" data-card-image="${imageFor(up.name)}" aria-label="Agrandir la carte ${fr}"><img src="${imageFor(up.name)}" alt="Carte ${fr}" onerror="this.closest('.upgrade-visual').classList.add('missing')"><span>Agrandir</span></button><div class="upgrade-copy"><small>AMÉLIORATION ${index+1}</small><h3>${fr}</h3>${defs.length?defs.map(item=>`<div class="upgrade-definition"><strong>${item.def.name}${item.tag.value!=null?' '+item.tag.value:''}</strong><p>${definitionText(item)}</p></div>`).join(''):`<p class="unavailable">Aucune définition rattachée dans le référentiel. Consultez le texte de la carte.</p>`}</div></article>`}).join('')}</div></section>`}
...
$('#restart').onclick=()=>{attacker=null;defender=null;stage=1;pick('attacker')};pick('attacker');
