(()=>{
  ...
  function list(){
    const pending=profiles().filter(([,p])=>pendingFor(p)>0),count=batchCount();
    return `<section class="cert-page"><header><div><span class="kicker">CONTRÔLE CENTRALISÉ</span><h1>Certification des dés</h1><p>${pendingTotal()} élément(s) restent à comparer aux cartes.</p></div><button class="secondary" id="closeCertification">Retour à l’assistant</button></header><div class="cert-batch-bar"><strong>${count} correction(s) dans le lot</strong><span>Vous pouvez contrôler toutes les cartes avant l’envoi.</span><button class="primary" id="sendBatch" ${count?'':'disabled'}>Envoyer toutes les corrections (${count})</button></div><div class="cert-list">${pending.map(([card,p])=>{const queued=(drafts[card]?.weapons||[]).filter(w=>w.queued).length+(drafts[card]?.defenseQueued?1:0);return `<button data-cert-card="${card}"><img src="${imageFor(card)}" alt=""><span><strong>${displayName(card)}</strong><small>${pendingFor(p)} contrôle(s) restant(s)${queued?` · ${queued} dans le lot`:''}</small></span><b>›</b></button>`}).join('')||'<p class="notice">Tous les profils sont certifiés.</p>'}</div></section>`;
  }
  function render(){
  ...
  function proposeBatch(){
    ...
    window.open(`https://github.com/${repository}/issues/new?title=${encodeURIComponent(`[Certification dés] Lot de ${batchCount()} corrections`)}&body=${encodeURIComponent(body)}`,'_blank','noopener');
  }
