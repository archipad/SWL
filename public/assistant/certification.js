(()=>{
  ...
  function reviewScreen(){
    ...
    return `<section class="cert-review"><header><div><span class="kicker">DÉFILÉ RAPIDE · ${position} / ${cards.length}</span><h2>${displayName(card)}</h2></div><button class="secondary" id="reviewExit">Retour à la liste · ${batchCount()} dans le lot</button></header><div class="cert-review-body"><div class="cert-card-visual"><img src="${imageFor(card)}" alt="Carte ${displayName(card)}"><small>Comparez chaque ligne du récapitulatif au visuel.</small></div><div class="cert-review-side">${aiPanel(card)}${hqPanel(card,d)}${sourcesPanel(card,d)}${weaponGapPanel(card,d)}${extra}${why.length?'<p class="notice">Avant de valider : '+why.join(' ; ')+'.</p>':''}<div class="cert-review-actions"><button class="primary big-confirm" id="reviewOk" ${ok?'':'disabled'}>✓ CONFORME · carte suivante</button><button class="secondary" id="reviewEdit">✎ À corriger…</button><button class="secondary" id="reviewSkip">Passer →</button></div></div></div></section>`
  }
